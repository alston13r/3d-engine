interface Triangle {
  vertecies: [number, number, number],
  reference: Vector[],
  color: string
...
class Triangle {
  constructor(p1: number, p2: number, p3: number, reference: Vector[], color: string = '#fff') {
    this.vertecies = [p1, p2, p3]
    this.reference = reference
    this.color = color
  }

  get p1(): Vector {return this.reference[this.vertecies[0]]}
  get p2(): Vector {return this.reference[this.vertecies[1]]}
  get p3(): Vector {return this.reference[this.vertecies[2]]}
  set p1(p: Vector) {
    if (this.reference.includes(p)) this.vertecies[0] = this.reference.indexOf(p)
    else this.vertecies[0] = this.reference.push(p)-1
  }
  set p2(p: Vector) {
    if (this.reference.includes(p)) this.vertecies[1] = this.reference.indexOf(p)
    else this.vertecies[1] = this.reference.push(p)-1
  }
  set p3(p: Vector) {
    if (this.reference.includes(p)) this.vertecies[2] = this.reference.indexOf(p)
    else this.vertecies[2] = this.reference.push(p)-1
  }
}
